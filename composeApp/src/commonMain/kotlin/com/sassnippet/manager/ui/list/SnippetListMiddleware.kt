...
class SnippetListMiddleware(
    ...
    override suspend fun process(
        ...
        when (intent) {
            is SnippetListIntent.LoadSnippets -> handleLoadSnippets(next)
            is SnippetListIntent.Search -> handleSearch(intent, next)
            else -> next(intent)
        }
    }

    private suspend fun handleLoadSnippets(
        next: suspend (SnippetListIntent) -> Unit
    ) {
    ...
    private suspend fun handleSearch(
        intent: SnippetListIntent.Search,
        next: suspend (SnippetListIntent) -> Unit
    ) {
